tSVDdct <- function (tnsr)
  {
    ...
    for (j in 1:n3) {
      decomp <- svd(dctz[, , j], nu = n1, nv = n2)
      U_arr[, , j] <- decomp$u
      V_arr[, , j] <- decomp$v
      S_arr[, , j] <- diag(decomp$d, nrow = n1, ncol = n2)
    }
    U <- as.tensor(aperm(apply(U_arr, MARGIN = 1:2,idct), c(2,3,1)))
    V <- as.tensor(aperm(apply(V_arr, MARGIN = 1:2,idct), c(2,3,1)))
    S <- as.tensor(aperm(apply(S_arr, MARGIN = 1:2,idct), c(2,3,1)))
